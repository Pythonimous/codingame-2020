...
# TODO: почему мы не получаем приказы?
...
class OurPacman(Pacman):
    ...
    def single_tile(self, path_scores, paths):
        """Choose one tile in the most valuable direction"""
        ...
        for goal, path in paths.items():
            for neighbor in my_neighbors:
                if path[position] == neighbor:
                    candidate_dict[neighbor] += path_scores[goal][0]
                    continue

        x, y = max(candidate_dict, key=candidate_dict.get)

        remaining_paths = {goal: path for goal, path in paths.items() if path[position] == (x, y)}
        remaining_scores = {goal: score[0] for goal, score in path_scores.items() if goal in remaining_paths}

        best_path = max(remaining_scores, key=remaining_scores.get)
        self.current_route = remaining_paths[best_path]
        self.current_goal = (best_path, remaining_scores[best_path])
        print(best_path, file=sys.stderr)
    ...
    def choose_pellet(self, grid, pacmen):
        ...
        sorted_scores, path_scores, paths = self.evaluate_paths(grid, pacmen)
    ...
    def not_cornered(self, sorted_scores, path_scores, paths):
        ...
        elif self.stuck:  # если мы застряли и не имеем цели
            self.single_tile(path_scores, paths)
    ...
    if turns == 1:
        ...
        for pacman in my_pacs.pacmen:
            ...
            if pacman.previous_coords[2]:
                if (pacman.previous_coords[2] in (
                pacman.previous_coords[0], pacman.coordinates)) and not pacman.fighting:
